...
Page({
    ...
    prepare(){
        ...
        var data = {}
            ...
            orderprepare(data).then(json => {
                wx.hideLoading()
                if (json.code == 1) {
                    var products = trail.fixListImage(json.data.products, 'product_image')
    ...
    calcolation: function () {
        let total_price = 0
        const expresses=this.data.express.postages
        //const postage={fee:0,posareas:{}}
        const poscalc={}
        ...
        for (var pid in poscalc){
            ...
            for(var aid in poscalc[pid].areas){
                ...
                if (area.free_limit > 0 && poscalc[pid].amount >= area.free_limit){
                    ...
                    let fee = util.force_number(area.first_fee)
                    let extend_fee = util.force_number(area.extend_fee)
                    if (poscalc[pid].total > area.first && area.extend > 0 && extend_fee > 0){
